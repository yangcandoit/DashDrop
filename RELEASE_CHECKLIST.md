...
### 1. 产品功能完整度
- [~] 传输任务管理增强：重试、批量取消、失败项重传（**已实现重试与批量取消；失败项“按文件级重传”仍未实现**）。
...
- [~] 关键指标采集：成功率、平均耗时、失败分布、取消率（**已实现终态计数与收发字节累计；平均耗时与分布统计仍未实现**）。
...
- [~] 错误提示可操作化（给出下一步建议）（**关键错误已结构化并可持久展示；统一“下一步操作建议”模板未完全覆盖**）。

### 2. 架构与长期维护
- [ ] 持久化方案统一评估（JSON vs tauri store vs sqlite）并固化（**未实现：当前仍为 JSON + SQLite 并存；但 `persistence.rs` 已升级为原子写入**）。
