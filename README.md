...
- [ ] mDNS 局域网广播与设备发现（`_dashdrop._udp.local`）
- [ ] QUIC + TLS 1.3 加密传输通道
- [ ] Ed25519 设备长期密钥对（系统凭据存储）
- [ ] 单/多文件发送（顺序传输 + BLAKE3 整文件校验）
- [ ] 接收请求队列卡片（Transfers 页处理，Accept / Decline）
- [ ] 主界面设备卡片列表（实时发现、上下线感知）
- [ ] 拖拽发送（文件拖到设备卡片）
- [ ] 传输进度条与完成通知
...
```typescript
// 前端调用后端
invoke('get_device_info')                          // 返回本机 name/fingerprint
invoke('get_devices')                              // 返回当前在线设备列表
invoke('send_files', { peer_fp, file_paths })      // 发起传输
